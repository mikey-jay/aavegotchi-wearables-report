...
def get_forge_purchases_from_database() -> pd.DataFrame:
    return db.get_df_from_database(FORGE_PURCHASES_TABLE_NAME).set_index('id')

def store_forge_purchases_to_database(df: pd.DataFrame):
    return db.store_df_to_database(FORGE_PURCHASES_TABLE_NAME, df)

def get_forge_purchases_df():
    if datetime.datetime.fromtimestamp(UPDATE_TIME) > db.get_table_modified_time(FORGE_PURCHASES_TABLE_NAME):
        forge_purchases_df = get_forge_purchases_from_subgraph()
        store_forge_purchases_to_database(forge_purchases_df)
    else:
        forge_purchases_df = get_forge_purchases_from_database()    
    return forge_purchases_df

def get_forge_purchases_types_df(forge_purchases_df, forge_item_types_df):
    return forge_purchases_df.merge(forge_item_types_df, left_on='erc1155TypeId', right_index=True, how='left')

def get_forge_purchases_from_subgraph():
    ...
    for interval_start in get_time_intervals(START_TIME, END_TIME, QUERY_TIME_INTERVAL):
        ...
        purchases_query = get_core_matic_query(
            { 'erc1155Purchases': {
                'params': { 'where': { 'category_in': ['7', '8', '9', '10', '11'], 'timeLastPurchased_lt': interval_end, 'timeLastPurchased_gte': interval_start } },
